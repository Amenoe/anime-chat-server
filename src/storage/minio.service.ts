...
export class MinioService implements OnModuleInit {
  ...
  async fPutObject(
    ...
    try {
      ...
      this.logger.error(
        `fPutObject failed: ${error instanceof Error ? error.message : error}`,
      );
      throw new InternalServerErrorException('上传播放缓存到对象存储失败');
    }
  }
  ...
  /** 按 Range 读对象片段（可选；播放优先走本地文件） */
  async getObjectPartial(objectName: string, offset: number, length: number) {
    try {
      ...
      const code = error?.code || error?.name;
      if (code === 'NoSuchKey' || code === 'NotFound') {
        throw new NotFoundException('播放文件不存在');
      }
      throw new InternalServerErrorException('读取播放文件失败');
    }
  }
}
