 ...
 */
export function toNumbers<T extends Record<string, unknown>>(rows: T[], keys: string[]): T[] {
  return rows.map((row) => {
...
/** 单行版本 */
export function rowToNumbers<T extends Record<string, unknown>>(row: T, keys: string[]): T {
